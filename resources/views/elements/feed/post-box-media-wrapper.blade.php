...
@endif
    ...
    @if($isGallery)
        @if(AttachmentHelper::getAttachmentType($attachment->type) == 'image')
            ...
            @elseif(AttachmentHelper::getAttachmentType($attachment->type) == 'video')
                <div class="video-wrapper h-100 w-100 d-flex justify-content-center align-items-center">
                    <video class="video-preview w-100" style="height: 550px;" src="{{$attachment->path}}" poster="{{$attachment->blob_pic}}" preload="metadata"  controls></video>
                </div>
        ...
        @else
            @if(AttachmentHelper::getAttachmentType($attachment->type) == 'image')
                <img src="{{$attachment->path}}" draggable="false" alt="" class="img-fluid rounded-0 w-100" style="max-height: 650px;max-width: 100%;object-fit: cover;">
            @elseif(AttachmentHelper::getAttachmentType($attachment->type) == 'video')
                <div class="video-wrapper h-100 w-100 d-flex justify-content-center align-items-center">
                    <video class="video-preview w-100" style="height: 550px;" src="{{$attachment->path}}"  poster="{{$attachment->blob_pic}}" preload="metadata" controls></video>
                </div>
